...
func (beacon *Beacon) Register(signals <-chan os.Signal, ready chan<- struct{}) error {
	...
	if beacon.RebalanceTime == 0 {
		rebalanceDuration = 1 * time.Hour
	}
	rebalanceTicker := time.NewTicker(rebalanceDuration)
	defer rebalanceTicker.Stop()

	// If RegistrationMode is direct, additional connections are not desired. Similarly, if RebalanceTime is not set, additional connections are not desired.
	if beacon.RebalanceTime == 0 || beacon.RegistrationMode == Direct{
		rebalanceTicker.Stop()
	}

	keepAliveFunc := func(channelToClose chan struct{}){
		if beacon.RebalanceTime != 0 && beacon.RegistrationMode == Forward{
			time.Sleep(beacon.RebalanceTime)
	...
	registerWorker := func(errChan chan error) {
		...
		// TODO CC: we can maybe remove this and instead
		//	   create a timeout'able context.
		go keepAliveFunc(disableKeepAlive)
	...
	for {
		...
		case <-rebalanceTicker.C:
			if wgCount < 5 && beacon.RegistrationMode != Direct {
				wg.Add(1)
				wgMutex.Lock()
				wgCount++
				wgMutex.Unlock()
